...
protest_income_df = 
  left_join(usa_protest, income_df, 
...
Joining all the ACS data with the protest data:
...
mpv_demo_df = 
  left_join(mpv_final3, race_df, 
            by = c("county" = "county", "state_name" = "state")) %>% 
  select(-c(lat.y, lng.y, prop_white, prop_black, prop_am_in_alask, prop_hawaii_pi, prop_other, prop_asian)) %>%
  pivot_longer(
    20:25,
    names_to = "total_race",
    values_to = "total"
...
write_csv(mpv_demo_df, "./data/mpv_race_df.csv")
