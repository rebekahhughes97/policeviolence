...
For the ACS data we looked at Race, Income, and Age by Sex; for each of these categories there was a separate dataset. All datasets have a tidy and untidy version in order for different visualization and exploratory analysis needs. However, for both the tidy and untidy versions, variable names were shortened for convenience and only a subset of of the original datasets' categories were kept (for example, in all datasets 'margin of error estimates' were removed. Below are the specific data cleaning steps for each dataset:
