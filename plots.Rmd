...
total_race =
  ...
  mutate(
    prop = total_estimate / total_pop * 100
  ) %>%
  ggplot(
...
race_mpv =
  ...
  mutate(
    ...
    race = str_replace_all(race, "NA", "Unknown race")
    ) %>%
  ggplot(aes(x = race, y = race, color = race, fill = race)) +
  geom_bar(stat = "identity") +
  theme(
...
# Protest and ACS Joint Dataset

Read in Protest and ACS datasets
```{r}
protest_income_df =
  mpv_demo_df =
...
protest_demo_df =
  mpv_demo_df =
    read_csv("./data/protest_demo_df.csv")
```
